...
public class SimpleLightningBolt : MonoBehaviour {

	[HideInInspector] public enum LightningBoltAnimationMode {
		None,
		Random,
		Loop,
		PingPong
    ...
    [Range(1, 16)] public int Rows = 1;
    [Range(1, 16)] public int Columns = 1;
    ...
    public void RandomVector(ref Vector3 start, ref Vector3 end, float offsetAmount, out Vector3 result) {
        if (Camera.main != null && Camera.main.orthographic) {
            ...
            float distance = (Random.value * offsetAmount * 2.0f) - offsetAmount;
            ...
            float distance = ((Random.value + 0.1f) * offsetAmount);

            // get random rotation angle to rotate around the current direction
            float rotationAngle = (Random.value * 360.0f);
    ...
    void UpdateLightPosition() {
		...
		pointLight.transform.position = segments[Random.Range(0, segments.Count)].Value;
	}
