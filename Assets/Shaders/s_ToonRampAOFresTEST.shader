Shader "Landon/Toon/RampAOFresTEST" {
	...
	SubShader {
		...
		inline half4 LightingToonyColorsCustom (SurfaceOutputCustom s, half3 lightDir, half3 viewDir, half atten) {
			...
			//AO stored in alpha
			ramp = saturate((ramp + rimColor) * s.Alpha);//FresGlow
			//ramp = saturate((ramp) * s.Alpha);//NoFresGlow
		...
		void surf (Input IN, inout SurfaceOutputCustom o) {
			...
			half3 color = rim * _RimColor.rgb  +  (1-rim) * _Color.rgb;

			//o.Albedo = color;

			o.Albedo = mainTex.rgb + (1 - mainTex.a) * color;
